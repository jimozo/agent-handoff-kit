...
This file is a short operating guide for Codex and other coding agents. It should contain only rules useful in every session. Read project details from linked docs only when the task needs them.
...
Use your own agent prefix if you are not Codex, for example `claude/`, `cursor/`, or `aider/`. If the human owner asks you to continue an existing branch, stay on it after confirming the branch and worktree state.
...
If session-log conflicts happen, preserve every entry and order them newest-first. If the handoff protocol doc defines an Agent Ownership Map, respect it: edit another agent's owned paths only when the task requires it, and never reformat them.
...
- Agent-switch relay note: [CONTINUE.md](./CONTINUE.md)
